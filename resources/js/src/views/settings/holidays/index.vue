      ...
      <div class="mb-5">
        <div class="vx-row">
          <div class="vx-col sm:w-1/4 w-full mb-4">
            ...
            <vs-input class="w-full" placeholder="Tên ngày nghỉ lễ" v-model="searchData.keyword"></vs-input>
          </div>
          <div class="vx-col sm:w-1/4 w-full mb-4">
        ...
        <div class="vx-row mt-3">
          <div class="vx-col w-full">
            <router-link class="btn btn-success" :to="'/settings/holidays/add'">
            ...
            <table class="vs-table vs-table--tbody-table">
              <thead class="vs-table--thead">
                <tr>
                  ...
                  <th colspan="1" rowspan="1">Ngày nghỉ lễ</th>
              ...
              <tr class="tr-values vs-table--tr tr-table-state-null" v-for="(item, index) in holidays" :key="index">
                <!---->
                ...
                <td class="td vs-table--td text-center">{{item.end_date}}</td>
                ...
                <td class="td vs-table--td text-center list-action"> 
                    <router-link :to="`/settings/holidays/detail/${item.id}`" >
                      <vs-button size="small"><i class="fa fa-eye"></i></vs-button>
                    </router-link> 
                    <router-link :to="`/settings/holidays/edit/${item.id}`" v-if="!item.disabled_delete">
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        ...
        this.$vs.loading()
        axios.p('/api/settings/holidays/list', data)
          .then((response) => {
            this.$vs.loading.close()
            this.holidays = response.data.list
      ...
      confirmDelete (item) {
        ...
        this.$vs.dialog({
          ...
          color: 'danger',
          title: 'Thông báo',
          text: `Bạn chắc chắn muốn xóa ngày nghỉ lễ - ${item.name}?`,
      ...
      deletetuition_fee(){
        ...
        this.$vs.loading();
        axios.p(`/api/settings/holidays/delete`,data)
        .then((response) => {
          this.$vs.loading.close();
          this.getData();
          this.$vs.notify({
